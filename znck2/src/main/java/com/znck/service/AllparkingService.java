...
public class AllparkingService {
        ...
        if (!StringUtils.isNullOrEmpty(getOutTimeString)) {
            ...
            parking.setFetureSpaceId(InitDataListener.spaces.stream()
                .filter(space -> 
                    space.getNature()
                    .equals(
                        contrastServiceImpl.getContrastByRealName("出口").getId()
                    )
    ...
    public void getCar() throws ParseException {
        ...
        List<
            ParkingEntity> inCar
                = InitDataListener.parkings.stream()
    ...
    public void SystemoutParkingInfo(){
        ...
        System.out.println("入口："+InitDataListener.spaces.stream().filter(space -> space.getNature().equals(contrastServiceImpl.getContrastByRealName("入口").getId())).collect(Collectors.toList()).size());
        System.out.println("出口："+InitDataListener.spaces.stream().filter(space -> space.getNature().equals(contrastServiceImpl.getContrastByRealName("出口").getId())).collect(Collectors.toList()).size());
        System.out.println("车库-占用"+InitDataListener.spaces.stream().filter(space -> space.getNature().equals(contrastServiceImpl.getContrastByRealName("车库-占用").getId())).collect(Collectors.toList()).size());
        System.out.println("通道-占用"+InitDataListener.spaces.stream().filter(space -> space.getNature().equals(contrastServiceImpl.getContrastByRealName("通道-占用").getId())).collect(Collectors.toList()).size());
        System.out.println("车库-空置"+InitDataListener.spaces.stream().filter(space -> space.getNature().equals(contrastServiceImpl.getContrastByRealName("车库-空置").getId())).collect(Collectors.toList()).size());
        System.out.println("通道-可通过"+InitDataListener.spaces.stream().filter(space -> space.getNature().equals(contrastServiceImpl.getContrastByRealName("通道-可通过").getId())).collect(Collectors.toList()).size());
    ...
    public void takeOutCarNextTodo(ParkingEntity parking) throws ParseException, InterruptedException {
        ...
        SpaceEntity nextSpace = InitDataListener.spaces.stream().filter(space -> space.getX() == nextSpaceMap.get(x)
            && space.getY() == nextSpaceMap.get(y) && space.getZ() == nextSpaceMap.get(z)).collect(Collectors.toList())
            .get(0);
        if (StringUtils.isNullOrEmpty(nextSpace.getCarId())) {
            ...
            if (nextSpaceMap.get(x) == fetureSpace.getX() && nextSpaceMap.get(y) == fetureSpace.getY()
                ...
                String way
                    = parking.getWay() + "|" + fetureSpace.getX() + "," + fetureSpace.getY() + "," + fetureSpace.getZ();
